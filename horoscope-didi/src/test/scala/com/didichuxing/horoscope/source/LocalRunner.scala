...
class LocalRunner extends Logging {
  ...
  def run(config: Config): Unit = {
    ...
    val flowManager = Horoscope.newLocalService()
      ...
      .withCompositorFactory("apollo_toggle", new ApolloToggleCompositorFactory)
      .withCompositorFactory("apollo_config", new ApolloConfigCompositorFactory)
      .withCompositorFactory("restful", new RestfulCompositorFactory(config))
      .withCompositorFactory("mlflow", new MLFlowCompositorFactory(config))
