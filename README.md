...
# 项目简介
随着大数据和人工智能的不断发展，越来越多的业务提出了对实时数据进行价值发现的需求。例如客服投诉处理，交通事件快速发现，舆情监控等。这些场景中，数据处理的越早，业务价值越高。而实时数据挖掘有很大的挑战，主要表现为以下几个方面：
...
综合上述原因, 我们开发了星盘(Horoscope), 
为以上三方面的挑战提供一个综合的系统化方案。**星盘是基于模型编排的实时数据挖掘框架，主要定位是辅助构建云原生场景下的智能信息处理和决策系统**。

星盘的主要贡献如下：
+ **交互式**: 针对数据的时变性和易失性，采用交互式的方法，开发者观察实时环境中的数据，动态调整算法逻辑，然后再观察和调整, 这是一种"Human-In-The-Loop"的过程。
+ **按需热埋点**: 数据挖掘的流程往往是多个算法过程的串联， 
  端到端的算法可以用反向传播-梯度下降来自动优化效果，而由胶水代码串起来的多个算法模型就无法如此优化，星盘设计了一种按需热埋点的机制，支持跨流程和分主题埋点，解决流程难监控和分析的问题。
+ **流程编排**: 对于挖掘过程中多服务交互的开发效率问题，星盘设计了一种"代码化的流程图"语言，为开发者提供low-code的数据挖掘体验。
...
综上所述, 业界主流的开源系统中, 目前还没有能同时完成实时流程编排,日志埋点和实验功能的实时数据挖掘系统, 星盘的优势正在于此。然而, 星盘系统也存在局限性, 例如:
