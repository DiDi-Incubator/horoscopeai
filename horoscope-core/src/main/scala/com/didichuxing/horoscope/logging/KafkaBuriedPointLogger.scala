...
class KafkaBuriedPointLogger(config: Config) extends Logging with LogHelper {
  import scala.collection.JavaConverters._
  private val kafkaConfig = config.getConfig("horoscope.buried-point-logger.kafka")
  ...
  info(("msg", "KafkaBuriedPointLogger started"))
  ...
  private def sendRecord(topicRecord: Topic): Unit = {
    ...
    producer.send(record, new Callback {
      override def onCompletion(metadata: RecordMetadata, exception: Exception): Unit = {
        if (exception != null) {
          logError(("msg", s"Failed to send buried point log "), ("topic", topic),
            ("metadata", metadata.toString), ("logdata", jsonLog), ("ex", exception.toString))
        } else {
          logDebug(("msg", s"Successfully send buried point log to kafka"), ("topic", topic), ("logdata", jsonLog))
        }
      }
    })
  }
  ...
  private def getProducerConfig(kafkaConfig: Config): Properties = {
    try {
      ...
      kafkaConfig.entrySet().asScala.foreach { entry =>
        ...
        info(("msg", "kafka buried point logger config"), (entry.getKey, entry.getValue.unwrapped().toString))
      }
      ...
      case e: Throwable =>
        throw new RuntimeException("Can't init kafka buried point logger", e)
    }
  }
  ...
  Runtime.getRuntime.addShutdownHook(new Thread {
    override def run(): Unit = {
      try {
        if (producer != null) {
          producer.close()
          logInfo(("msg", "close buried point kafka producer"), ("topic", topic))
        }
      } catch {
        case e: Throwable =>
          e.printStackTrace()
          logError(("msg", "close buried point kafka producer error"), ("ex", e.toString))
      }
    }
  })
